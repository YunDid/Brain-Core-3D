...
class TimestampReader(DataReader):
    ...
    def read(self, file_descriptor, num_samples):
        """读取时间戳数据"""
        if self.use_mmap == True and num_samples > 4096:
            ...
            # 回退offset，重读相同位置
            file_path = file_descriptor.name
            if file_path in self.mmap_states:
                state = self.mmap_states[file_path]
                state['offset'] -= len(data) * 2  # 回退
                
                # 第二次读取相同数据
                t3 = time.perf_counter()
                data2 = self._read_from_mmap(file_descriptor, num_samples, np.int16, 2)
                t4 = time.perf_counter()
                second_read = (t4 - t3) * 1000
                
                self._logger.debug("mmap Timestamp_Nobreak read_delay {} ms", second_read)
        else:
            t_start = time.perf_counter()  # 开始时间点
    ...
    def read(self, file_descriptor, num_samples):
        ...
        if self.use_mmap == True and num_samples > 4096:
            ...
            if file_path in self.mmap_states:
                ...
                self._logger.debug("mmap AmpData_Nobreak read_delay {} ms", second_read)
        else:
            t_start = time.perf_counter()  # 开始时间点
